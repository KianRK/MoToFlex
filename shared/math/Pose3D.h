...
class Pose3D : public Streamable
{
  ...
  */
  ...
  /** Multiplication with another transformation, 
  *   handled like a 4x4 matrix multiplication of transformations in homogeneous coordinates
  *\param other The other Pose3D that will multiplied with this one
  */
  Pose3D operator*(const Pose3D& other) const
  {
    Pose3D result;
    result.rotation = this->rotation * other.rotation;
    result.translation = this->translation + (this->rotation * other.translation);
    return result;
  }
  ...
  */
  ...
  /** Calculates the inverse transformation from the current pose, but not at all in a correct way!
  *   Nevertheless this is used in the cameraMatrix calculation ever since...
  * @return The inverse transformation pose.
  */
  Pose3D invertRotationAndReverseTranslation() const
  {
    Pose3D result;
  ...
  */
  Pose3D invert() const
  {
    Pose3D result;
    result.rotation = this->rotation.transpose();
    result.translate(this->translation * -1);
    return result;
  }
