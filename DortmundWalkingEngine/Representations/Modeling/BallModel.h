...
class BallModel : public Streamable
{
  ...
  virtual void serialize(In *in, Out *out)
  {
    STREAM_REGISTER_BEGIN();
      ...
      STREAM(timeWhenLastSeenByTeamMate);
    STREAM_REGISTER_FINISH();
  }

public:
  /** Constructor */
  BallModel() : timeWhenLastSeen(0),timeWhenLastSeenByTeamMate(0) {}
  ...
  unsigned timeWhenLastSeenByTeamMate; /**< Time stamp, indicating what its name says*/
...
class BallModelAfterPreview : public BallModel
{
public:
  void draw() 
  {
    DECLARE_DEBUG_DRAWING("representation:BallModelAfterPreview", "drawingOnField"); // drawing of the ground truth ball model
    COMPLEX_DRAWING("representation:BallModelAfterPreview", 
    {
      const Vector2<double>& position(lastPerception.position);
      const Vector2<double>& velocity(estimate.velocity);
      CIRCLE("representation:BallModelAfterPreview", 
        position.x, 
        position.y, 
        45, 
        1, // pen width 
        ...
        Drawings::bs_solid, 
        ColorRGBA(255, 128, 0, 192));
      ARROW("representation:BallModelAfterPreview", position.x, position.y, 
        position.x + velocity.x, position.y + velocity.y, 5, 1, ColorRGBA(255, 128, 0, 192));
    });
  }
};

class LocalBallModel : public BallModel
{
public:
  void draw() 
  {
    DECLARE_DEBUG_DRAWING("representation:LocalBallModel", "drawingOnField"); // drawing of the ground truth ball model
    COMPLEX_DRAWING("representation:LocalBallModel", 
    {
      const Vector2<double>& position(lastPerception.position);
      const Vector2<double>& velocity(estimate.velocity);
      CIRCLE("representation:LocalBallModel", 
        position.x, 
        position.y, 
        45, 
        1, // pen width 
        Drawings::ps_solid, 
        ColorRGBA(255, 0, 255, 192),
        Drawings::bs_solid, 
        ColorRGBA(255, 0, 255, 192));
      ARROW("representation:LocalBallModel", position.x, position.y, 
        position.x + velocity.x, position.y + velocity.y, 5, 1, ColorRGBA(255, 0, 255, 192));
    });
  }
};
