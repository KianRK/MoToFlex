...
public:
	...
	void Start() { useSensor(theWalkingEngineParams.sensorControlRatio); }
	 ...
	 */
	void getObservations(Vector3<double> &x, Vector3<double> &y);

	Vector2<double> getReferenceZMP();
...
private:
	...
	const JointData				&theJointData;			/**< Set by constructor. */
	const ActualCoM				&theActualCoM;			/**< Set by constructor. */

	typedef list<ZMP *> ZMPList;
	...
	double cur_x[3],		/**< Controller state vector for x axis. */
	...
	double cont_x,			/**< Controller internal value. */
		cont_y;				/**< Controller internal value. */

	double vx,				/**< Controller internal value. */
