...
double Geometry::Line::distancePointToLine(const Vector2<double> & point) const
{
  Vector2<double> normal = direction;
  normal.rotateRight();
  normal.normalize();
  double distance = (point - base) * normal;
  return abs(distance);
}
...
double Geometry::distancePointToLine(
    const Line& line,
    const Vector2<double>& point
    )
{
  return line.distancePointToLine(point);
}
...
// introduced by some BHuman student... Basically the same as calculatePointOnField without distance check at the end
...
{
  Vector3<double> vectorInCameraCoords( cameraInfo.focalLength,
                                        cameraInfo.opticalCenter.x - x,
                                        cameraInfo.opticalCenter.y - y);

  const Vector3<double>& vectorInWorldCoords(cameraMatrix.rotation * vectorInCameraCoords);

  const double scale( - cameraMatrix.translation.z / vectorInWorldCoords.z);

  //Is the point above the horizon ? - return
  if (scale <= 0) return false;


  pointOnField.x = cameraMatrix.translation.x + scale * vectorInWorldCoords.x;
  pointOnField.y = cameraMatrix.translation.y + scale * vectorInWorldCoords.y;

  return fabs(pointOnField.x) < 10000 && fabs(pointOnField.y) < 10000;
}


bool Geometry::calculatePointOnHorizontalPlane(const Vector2<double>& pointInImage,
                                               const double planeZ,
                                               const CameraMatrix& cameraMatrix,
                                               const CameraInfo& cameraInfo, 
...
{
  Vector3<double> vectorInCameraCoords( cameraInfo.focalLength,
                                        cameraInfo.opticalCenter.x - pointInImage.x,
                                        cameraInfo.opticalCenter.y - pointInImage.y);

  const Vector3<double>& vectorInWorldCoords(cameraMatrix.rotation * vectorInCameraCoords);

  //if(fieldZ < cameraMatrix.translation.z)
  //{
  //  if(vectorInWorldCoords.z > 0) return false;
  //}
  //else
  //{
  //  if(vectorInWorldCoords.z < 0) return false;
  ...
  const double scale((planeZ - cameraMatrix.translation.z) / vectorInWorldCoords.z);

  if (scale <= 0) return false;


  pointOnPlane.x = cameraMatrix.translation.x + scale * vectorInWorldCoords.x;
  pointOnPlane.y = cameraMatrix.translation.y + scale * vectorInWorldCoords.y;
  //pointOnPlane.z = fieldZ;
  return true;
}
...
void Geometry::calculatePointInImage(const Vector3<double>& pointInWorld, 
                                     const CameraMatrix& cameraMatrix, 
                                     const CameraInfo& cameraInfo,
                                     Vector2<double>& pointInImage)
{
...
double Geometry::calculateLineSizeDouble
(
 const Vector2<int>& pointInImage, 
 const CameraMatrix& cameraMatrix,
 const CameraInfo& cameraInfo,
 ...
 )
{
  ...
  if(Geometry::calculatePointOnField(pointInImage.x, pointInImage.y, cameraMatrix, cameraInfo, pointOnField))
  {
    double distance = sqrt(sqr(cameraMatrix.translation.z) + sqr(pointOnField.abs()));
...
double Geometry::calculateLineSize
(
 const Vector2<int>& pointInImage, 
 const Vector2<double>& normalInImage,
 const CameraMatrix& cameraMatrix,
 const CameraInfo& cameraInfo,
 double fieldLinesWidth
 )
{
	Vector2<double> startPointOnField, endPointOnField, orientationOnField; 
	Vector2<double> imgLineExtreme1, imgLineExtreme2, fieldLineExtreme1, fieldLineExtreme2;
	Vector2<int> endPoint(int(pointInImage.x + normalInImage.x * 10), int(pointInImage.y + normalInImage.y * 10));
	if(Geometry::calculatePointOnField(pointInImage.x, pointInImage.y, cameraMatrix, cameraInfo, startPointOnField))
	{
		if(Geometry::calculatePointOnField(endPoint.x, endPoint.y, cameraMatrix, cameraInfo, endPointOnField))
		{
			orientationOnField = (endPointOnField - startPointOnField);
			orientationOnField.normalize();
            fieldLineExtreme1 = startPointOnField + orientationOnField*fieldLinesWidth/2;
			fieldLineExtreme2 = startPointOnField - orientationOnField*fieldLinesWidth/2;
			calculatePointInImage(fieldLineExtreme1, cameraMatrix, cameraInfo, imgLineExtreme1);
			calculatePointInImage(fieldLineExtreme2, cameraMatrix, cameraInfo, imgLineExtreme2);
			return (imgLineExtreme2 - imgLineExtreme1).abs();
		}
	}
	return 0;
}