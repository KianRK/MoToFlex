...
public:
  ...
  struct Line
  {
    ...
    double distancePointToLine(const Vector2<double> & point) const;
  };
...
public:
  ...
  {
    calculateAnglesForPoint(Vector2<double>(point.x,point.y),cameraMatrix,cameraInfo,angles);
  }
  ...
  */
  ...
  { 
    ...
    pointOnField.x = (int)pointOnFieldDouble.x; // correct rounding considered unnecessary for 0.5mm
    pointOnField.y = (int)pointOnFieldDouble.y; // correct rounding considered unnecessary for 0.5mm
    return onField;
  }
  ...
  * @param pointInImage Specifies the image point to be projected to the horizontal plane.
  * @param planeZ Specifies the height of the horizontal plane.
  * @param cameraMatrix The camera matrix of the image.
  * @param cameraInfo The camera info of the image.
  * @param pointOnPlane The resulting point.
  */
  static bool calculatePointOnHorizontalPlane(
    const Vector2<double>& pointInImage, 
    const double planeZ, 
    const CameraMatrix& cameraMatrix, 
    const CameraInfo& cameraInfo, 
  ...
  static bool calculatePointOnHorizontalPlane(
    ...
    Vector2<double>& pointOnPlane)
  {
    return calculatePointOnHorizontalPlane(Vector2<double>(pointInImage.x,pointInImage.y),planeZ,cameraMatrix,cameraInfo,pointOnPlane);
  }
  ...
  static bool calculatePointOnHorizontalPlane(
    ...
    const CameraMatrix& cameraMatrix, 
    const CameraInfo& cameraInfo, 
    Vector3<double>& pointOnPlane)
  {
    Vector2<double> temp;
    bool result = calculatePointOnHorizontalPlane(pointInImage,planeZ,cameraMatrix,cameraInfo,temp);
    pointOnPlane.x = temp.x;
    pointOnPlane.y = temp.y;
    pointOnPlane.z = planeZ;
    return result;
  }
  ...
  */
  static int calculateLineSize(
    ...
    const CameraMatrix& cameraMatrix,
    const CameraInfo& cameraInfo,
    double fieldLinesWidth = 50.0
    );

  static double calculateLineSizeDouble
	  (
	  const Vector2<int>& pointInImage, 
	  const CameraMatrix& cameraMatrix,
	  const CameraInfo& cameraInfo,
  ...
  static double calculateLineSize
	  (
	  const Vector2<int>& pointInImage, 
