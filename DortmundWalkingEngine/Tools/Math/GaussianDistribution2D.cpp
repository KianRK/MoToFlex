...
double GaussianDistribution2D::normalizedProbabilityAt(const Vector2<double>& pos) const
{
  Vector2<double> diff(pos - mean);
  double exponent(diff*(covariance.invert()*diff));
                                                           ...
                                                           double& eValue1, double& eValue2) const
{
  ...
  if (covariance.c[1].y - eValue1 != 0.0) 
  {
    eVec1.x = 1.0;
    eVec1.y = -covariance.c[0].y / (covariance.c[1].y - eValue1);
    eVec1.normalize();
  }
  else if (covariance.c[0].y == 0.0)
  {
    eVec1.x = 1.0;
    eVec1.y = 0.0;
  }
  ...
  if (covariance.c[0].x - eValue2 != 0.0)
  {
    eVec2.x = -covariance.c[1].x / (covariance.c[0].x - eValue2);
    eVec2.y = 1.0;
    eVec2.normalize();
  }
  else if (covariance.c[1].x == 0.0)
  {
    eVec1.x = 0.0;
    eVec1.y = 1.0;
  }
