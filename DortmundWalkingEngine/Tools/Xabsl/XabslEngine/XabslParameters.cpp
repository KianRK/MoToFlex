...
void Parameters::logAllParam()
{
	DebugSymbolsLog::printWithoutSeperator("(");
	for(int i = 0; i < decimal.getSize(); i++)
	{
		DebugSymbolsLog::printStaticSymbol(std::string(decimal.getName(i)), *decimal.getElement(i));
	}

	for(int i = 0; i < boolean.getSize(); i++)
	{
		DebugSymbolsLog::printStaticSymbol(std::string(boolean.getName(i)), *boolean.getElement(i));
	}

	for(int i = 0; i < enumerated.getSize(); i++)
	{
		DebugSymbolsLog::printStaticSymbol(std::string(enumerated.getName(i)), *enumerated.getElement(i));
	}

	/*for(int i = 0; i < enumerations.getSize(); i++)
	{
		DebugSymbolsLog::printStaticSymbol(std::string(enumerations.getName(i)), enumerations.getElement(i));
	}		*/
	DebugSymbolsLog::printWithoutSeperator(")");
	DebugSymbolsLog::printSeparator();
}
