...
#ifdef TARGET_SIM
#include <windows.h>
static PROCESS_WIDE_STORAGE  LARGE_INTEGER _tstart, _tend;
static PROCESS_WIDE_STORAGE  LARGE_INTEGER freq;

void tstart(void)
{
	static PROCESS_WIDE_STORAGE  int first = 1;
	if(first) 
	{
...
#ifdef TARGET_ROBOT
#include <sys/time.h>
static PROCESS_WIDE_STORAGE  struct timeval _tstart, _tend;
static PROCESS_WIDE_STORAGE  struct timezone tz;
...
{
	#ifdef PROFILING
		char moduleFilterName[255];  	
		sprintf(moduleFilterName,"%s/Config/Logs/stats.%s",File::getGTDir(),process.c_str());
		modstats.open( moduleFilterName, std::ios::out ); 
		modstats << "ModuleName;FrameNr;Representation;TimeInMs\n";
		sprintf(moduleFilterName,"%s/Config/modules.%s",File::getGTDir(),process.c_str());
	#endif
...
{
	#ifdef PROFILING
		static PROCESS_WIDE_STORAGE  int frame=0;
		frame++;
	#endif
...
#ifdef PROFILING
		tend();
		modstats << i->moduleState->module->name << ";" << frame<<";"<< i->representation << ";";
		modstats << tval()*1000 << "\n";
		flush( modstats );
#endif
	}
